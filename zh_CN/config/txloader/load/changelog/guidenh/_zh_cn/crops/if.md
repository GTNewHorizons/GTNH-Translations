...
> [!NOTE]
> <Color id="GREEN">Fertilization Bonus </Color> is 1 if not fertilized, or 1.5 if fertilized
...
- <ItemImage id="cropsnh:cropsnh.fertilizerUnit:2" /> __Fertilization Unit (MV+)__: This unit requires the <Color id="GREEN">Industrial Farm</Color> to always consume <Color id="GREEN">Enriched Fertilizer</Color>, at the beginning of each cycle to boost output production. It also stops item fertilizer from being usable in the machine. This increases growth speed by x1.5, the number of drops by +50%, and power consumption by +50% but halts processing if it ever runs out. Higher tier units offer the exact same benefits but require more enriched fertilizer per cycle. There can only be 1 fertilization unit per structure
